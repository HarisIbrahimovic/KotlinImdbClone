...
class MovieRatingAdapter(private val context:Context): RecyclerView.Adapter<MovieRatingAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if(rating.score.toString()!="0.0")
            holder.score.text = rating.score.toString()
        else
            holder.score.text =nR

        val requestOptions = RequestOptions.centerCropTransform()
        if(rating.userPicturePath=="default")
            Glide.with(context)
                .load(Constants.NO_IMAGE_MOVIE)
                .apply(requestOptions)
                .into(holder.imageView)
        else
        Glide.with(context).load(rating.userPicturePath).into(holder.imageView)
    }
